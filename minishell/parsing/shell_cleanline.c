...
/*   Updated: 2025/07/06 01:02:16 by thcaquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*shell_clean_line(t_data *data, t_token *cur)
{
	t_index	in;
	...
	{
		if (in.i >= 0 && in.m != 1 && cur->str && cur->str[in.i] == '$'
			&& (ft_isalnum(cur->str[in.i + 1]) == 1
				|| cur->str[in.i + 1] == '?' || cur->str[in.i + 1] == '_'))
		{
			in.i = shell_expand(data, cur->str, in.i);
			in.w = 1;
		}
		if (in.w == 1)
			cur = shell_token_sep(data, cur, &in);
		if (cur->str[in.i])
		{
			shell_mode_switch(cur->str, &in, 1);
			in.i++;
		}
		if (in.w == 1 && in.w-- && in.i > 0)
			in.i--;
	}
	return (cur);
}
