...
int	mini_mode_switch(char *str, int i, int mode)
{	if (mode == 0 && str[i] == '"')
	{
...
void	mini_clean_line(t_data *data)
{
	...
	while (data->line != NULL && data->line[++i])
	{
		...
		mode = mini_mode_switch(data->line, i, mode);
		//if (mode != 0)
		//	i--;
		here = mini_redirection_switch(data->line, i, mode, here);
		if (here == -1)
			return (mini_invalid_line(data));
...
int	mini_countwords(char *str)
{
	...
	while (str[++in.i])
	{
		in.m = mini_mode_switch(str, in.i, in.m);
		if (in.m != 0)
			in.w++;
